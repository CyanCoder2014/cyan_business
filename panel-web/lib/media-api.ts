...
const assetBase="/api/platform/service/media-service/endpoint/media/assets";const scopeHeaders=(s:MediaScope)=>({"X-Tenant-Key":s.tenantKey,...(s.siteKey?{"X-Site-Key":s.siteKey}:{})});async function assetJson<T>(path:string,s:MediaScope,init:RequestInit={}){const r=await platformFetch(`${assetBase}${path}`,{...init,headers:{"Content-Type":"application/json",...scopeHeaders(s),...(init.headers??{})},cache:"no-store"});if(!r.ok)throw await platformErrorFromResponse(r);return r.json()as Promise<T>}export const listMediaAssets=(s:MediaScope,q="",type="")=>assetJson<MediaPage>(`?q=${encodeURIComponent(q)}&type=${encodeURIComponent(type)}`,s);export const getMediaAsset=(s:MediaScope,k:string)=>assetJson<MediaAsset>(`/${encodeURIComponent(k)}`,s);export const getMediaUsage=(s:MediaScope,k:string)=>assetJson<MediaUsage>(`/${encodeURIComponent(k)}/usage`,s);export const updateMediaAsset=(s:MediaScope,k:string,patch:UpdateAssetRequest)=>assetJson<MediaAsset>(`/${encodeURIComponent(k)}`,s,{method:"PATCH",body:JSON.stringify(patch)});
export const mediaPreviewUrl=(a:MediaAsset)=>a.visibility==="PUBLIC"?`/api/platform/service/media-service/public/media/content/${encodeURIComponent(a.assetKey)}`:null;
export const mediaContentPath=(a:MediaAsset)=>a.deliveryUrl?`/api/platform/service/media-service${a.deliveryUrl}`:null;
export const deleteMediaAsset=async(s:MediaScope,k:string)=>{const r=await platformFetch(`${assetBase}/${encodeURIComponent(k)}`,{method:"DELETE",headers:scopeHeaders(s)});if(!r.ok)throw await platformErrorFromResponse(r)};

...
export default function MemberFormPage() {
  ...
  const load = useCallback(async () => { if (!tenantKey) { setLoading(false); return; } setLoading(true); setError(null); try { setForm(await getMemberForm(slug, { tenantKey, siteKey })); } catch (cause) { setError(cause instanceof Error ? cause.message : "Form unavailable"); } finally { setLoading(false); } }, [slug, tenantKey, siteKey]);

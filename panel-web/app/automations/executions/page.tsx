...
export default function Executions() {
  ...
  const load = useCallback(() => { if (!tenantKey) return; setLoading(true); setError(null); listExecutions(undefined, status || undefined, scope).then(setItems).catch(cause => setError(cause instanceof Error ? cause.message : String(cause))).finally(() => setLoading(false)); }, [scope, status, tenantKey]);

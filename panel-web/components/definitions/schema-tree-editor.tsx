...
export function SchemaTreeEditor({ fields, locale, onChange }: Props) {
  ...
  return <div className="schema-tree-workspace">
    <aside className="schema-tree" aria-label={locale === "fa" ? "درخت ساختار موجودیت" : "Entity schema tree"}>
      ...
      <div className="schema-tree-scroll"><TreeLevel fields={fields} parent={[]} selected={selectedPath} expanded={expanded} setExpanded={setExpanded} select={setSelectedPath} add={openAdd} remove={setDeletePath} move={moveSibling} drag={setDragged} drop={drop} locale={locale}/></div>
    ...
    <main className="schema-overview">
      <header><div><span className="page-kicker">{locale === "fa" ? "طرح داده" : "Data shape"}</span><h2>{locale === "fa" ? "درخت فیلدها" : "Field hierarchy"}</h2></div><button className="secondary-pill" onClick={() => openAdd(selected && isContainer(selected) ? selectedPath : [])}>＋ {selected && isContainer(selected) ? (locale === "fa" ? "فیلد فرزند" : "Child field") : (locale === "fa" ? "فیلد" : "Field")}</button></header>
      {Object.keys(fields).length ? <SchemaOutline fields={fields} expanded={expanded} select={path => { setSelectedPath(path); setExpanded(current => new Set(current).add(pathKey(path.slice(0, -1)))); }}/>:<EmptyState title={locale === "fa" ? "هنوز فیلدی نیست" : "No fields yet"} description={locale === "fa" ? "اولین فیلد، شیء یا لیست را اضافه کنید." : "Add the first field, object, or list."} action={<button className="primary-pill" onClick={() => openAdd()}>＋ {locale === "fa" ? "افزودن فیلد" : "Add field"}</button>}/>}
    </main>
...
function SchemaOutline({ fields, expanded, select, parent = [] }: { fields: Record<string, SchemaField>; expanded: Set<string>; select: (path: string[]) => void; parent?: string[] }) {
  return <div className={parent.length ? "schema-outline-children" : "schema-outline-root"}>{Object.entries(fields).map(([key, field]) => { const path = [...parent, key]; return <article key={pathKey(path)}><button onClick={() => select(path)}><span className={`field-type-icon ${field.type}`}>{isContainer(field) ? "▦" : "•"}</span><span><strong>{key}</strong><small>{field.type ?? "string"} · {field.validations?.length ?? 0} rules · {field.operations?.length ?? 0} operations</small></span></button>{isContainer(field) && (expanded.has(pathKey(path)) || Object.keys(field.itemValidations ?? {}).length > 0) ? <SchemaOutline fields={field.itemValidations ?? {}} expanded={expanded} select={select} parent={path}/> : null}</article>; })}</div>;
}

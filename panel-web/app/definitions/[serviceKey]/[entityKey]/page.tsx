...
export default function DefinitionEditor() {
  ...
  return <PanelShell activeKey="maker" title={value?.title || entity} titleFa={value?.title || entity} subtitle={`${service} · ${entity}`} subtitleFa={`${service} · ${entity}`}>
    <div className="definition-editor-toolbar"><button className="secondary-pill" onClick={() => router.push("/definitions")}>← {locale === "fa" ? "تعریف‌ها" : "Definitions"}</button><div>{value?.active ? <><Link className="secondary-pill" href={`/data/${service}/${encodeURIComponent(entity)}`}>{locale === "fa" ? "فرم و داده" : "Form & data"}</Link><Link className="secondary-pill" href={`/forms?serviceKey=${encodeURIComponent(service)}&entityKey=${encodeURIComponent(entity)}`}>{locale === "fa" ? "سازنده فرم" : "Form maker"}</Link><Link className="secondary-pill" href={`/bpm/new?entityService=${encodeURIComponent(service)}&entityKey=${encodeURIComponent(entity)}`}>{locale === "fa" ? "استفاده در BPM" : "Use in BPM"}</Link></> : null}<button className="secondary-pill" disabled={Boolean(pending)} onClick={load}>{locale === "fa" ? "بارگذاری مجدد" : "Reload"}</button>{canManage?<><AsyncButton className="secondary-pill" pending={pending === "save"} pendingLabel={locale === "fa" ? "ذخیره…" : "Saving…"} disabled={!dirty || Boolean(pending)} onClick={save}>{locale === "fa" ? "ذخیره پیش‌نویس" : "Save draft"}</AsyncButton><button className="primary-pill" disabled={dirty || !value || Boolean(pending)} onClick={() => setPublishOpen(true)}>{locale === "fa" ? "انتشار" : "Publish"}</button></>:null}</div></div>

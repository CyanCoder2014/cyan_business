...
export const deleteMediaAsset=async(s:MediaScope,k:string)=>{const r=await platformFetch(`${assetBase}/${encodeURIComponent(k)}`,{method:"DELETE",headers:scopeHeaders(s)});if(!r.ok){const b=await r.json().catch(()=>null);throw new Error(b?.message??b?.detail??`Media delete failed (${r.status})`)}};

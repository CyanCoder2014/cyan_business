...
public class EndpointAutomationFlowController {
    ...
    public EndpointAutomationFlowController(AutomationFlowDefinitionService service, N8nWorkflowCompatibilityService n8n, AutomationAiAuthorizationService aiAuthorization) { this.service = service; this.n8n = n8n; this.aiAuthorization = aiAuthorization; }
    @PostMapping public AutomationFlowDefinition save(@RequestHeader(value="X-Tenant-Key", required=false) String tenant, @RequestHeader(value="X-Site-Key", required=false) String site, @RequestBody AutomationFlowDefinition definition, Authentication auth) { if(hasAi(definition))aiAuthorization.requireBuilder(tenant,site,auth==null?null:auth.getName());return service.save(tenant, site, definition, auth == null ? "system" : auth.getName()); }
    ...
    @GetMapping("/{flowKey}/versions/{version}/readiness") public Map<String,Object> readiness(@RequestHeader(value="X-Tenant-Key", required=false) String tenant,@RequestHeader(value="X-Site-Key",required=false) String site,@PathVariable String flowKey,@PathVariable Integer version){return service.readiness(tenant,site,flowKey,version);}
    @PostMapping("/{flowKey}/versions/{version}/{action}") public AutomationFlowDefinition lifecycle(@RequestHeader(value="X-Tenant-Key", required=false) String tenant, @RequestHeader(value="X-Site-Key", required=false) String site, @PathVariable String flowKey, @PathVariable Integer version, @PathVariable String action, @RequestParam(required=false) String targetEnvironment, Authentication auth) { AutomationFlowDefinition definition=service.get(tenant,site,flowKey,version);if(hasAi(definition))aiAuthorization.requireBuilder(tenant,site,auth==null?null:auth.getName());return service.lifecycle(tenant, site, flowKey, version, action, auth == null ? "system" : auth.getName(), targetEnvironment); }
    ...
    private boolean hasAi(AutomationFlowDefinition definition){return definition!=null&&definition.getNodes().stream().anyMatch(node->node.type()==AutomationNodeType.AI_OPERATION);}
}

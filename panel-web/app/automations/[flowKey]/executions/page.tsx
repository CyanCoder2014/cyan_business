...
export default function FlowExecutionsPage({ params }: { params: { flowKey: string } }) {
  const { tenantKey, siteKey, queryVersion } = useScopeAccess(); const scope = useMemo(() => ({ tenantKey: tenantKey ?? undefined, siteKey: siteKey ?? undefined }), [tenantKey, siteKey]);
  ...
  const load = useCallback(() => { if (!tenantKey) return; setLoading(true); setError(null); listExecutions(params.flowKey, undefined, scope).then(setItems).catch(cause => setError(cause instanceof Error ? cause.message : String(cause))).finally(() => setLoading(false)); }, [params.flowKey, scope, tenantKey]);

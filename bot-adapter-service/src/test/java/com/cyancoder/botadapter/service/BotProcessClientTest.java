...
class BotProcessClientTest {
    ...
    void dispatchesTelegramMessageToAutomationWithStableIdempotency() {
        ...
        BotProcessClient client = new BotProcessClient(rest, properties, credentialsResolver());
        ...
        server.expect(once(), requestTo("http://localhost:9120/internal/automation-orchestrator/executions/start"))
                ...
                .andExpect(header("Authorization", "Basic cnVudGltZS1hdXRvbWF0aW9uOnJ1bnRpbWUtc2VjcmV0"))
    ...
    void dispatchesBaleMessageToBpmWithExternalBotActor() {
        RestTemplate rest = new RestTemplate();
        MockRestServiceServer server = MockRestServiceServer.bindTo(rest).build();
        BotProcessClient client = new BotProcessClient(rest, new AiOrchestratorProperties(), credentialsResolver());
        ...
        server.expect(requestTo("http://localhost:9119/internal/bpm/managed-objects"))
                ...
                .andExpect(header("Authorization", "Basic YnBtX2ludGVybmFsOmJwbV9zZWNyZXQ="))
    ...
    private InternalServiceCredentialsResolver credentialsResolver() {
        return new InternalServiceCredentialsResolver(new MockEnvironment()
                .withProperty("AUTOMATION_ORCHESTRATOR_SERVICE_INTERNAL_USERNAME", "runtime-automation")
                .withProperty("AUTOMATION_ORCHESTRATOR_SERVICE_INTERNAL_PASSWORD", "runtime-secret"));
    }
}

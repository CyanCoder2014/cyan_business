...
import { attachProjectAsset, closeConversationSession, createConversationSession, generatePlatformApp, getClientDraft, getConversationSession, listBlueprints, listConversationSessions } from "@/lib/platform-api";
...
export default function AiPage() {
  ...
  return <PanelShell activeKey="studio" title="AI Studio" titleFa="استودیوی هوش مصنوعی" subtitle="Build through persisted conversations and backend-validated drafts." subtitleFa="با گفتگوهای پایدار و پیش‌نویس‌های معتبر backend بسازید.">
    <div className="ai-workspace">
      <aside className="ai-session-list"><button className="primary-pill" onClick={start}>{locale === "fa" ? "گفتگوی جدید" : "New conversation"}</button>{sessions.map((session) => <button key={session.sessionId} className={active?.sessionId === session.sessionId ? "active" : ""} onClick={() => setActive(session)}><strong>{session.latestPrompt || session.sessionId}</strong><span>{session.status}</span></button>)}</aside>

...
public class GraphAutomationRuntime {
    ...
    private NodeResult runNode(AutomationExecution ex,AutomationFlowDefinition def,AutomationNode node,AutomationExecutionStep step){
        ...
        return switch(node.type()){
            ...
            case AI_OPERATION->{Map<String,Object> response=runAiOperation(ex,c);String target=Objects.toString(c.getOrDefault("resultPath","aiResult"));AutomationDataSupport.setPath(ex.getOutput(),target,response.get("output"));finish(step,"COMPLETED",response,null);yield go(next(node.id(),null,edges));}
    ...
    private Map<String,Object> runAiOperation(AutomationExecution execution, Map<String,Object> config) {
        Map<String,Object> request = new LinkedHashMap<>();
        request.put("operation", Objects.toString(value(execution, config.get("operation")), ""));
        request.put("instructions", Objects.toString(value(execution, config.get("instructions")), ""));
        Object input = value(execution, config.get("input"));
        if (input != null) request.put("input", input);
        Object outputSchema = value(execution, config.get("outputSchema"));
        ...
        return AutomationDataSupport.map(raw);
    }
